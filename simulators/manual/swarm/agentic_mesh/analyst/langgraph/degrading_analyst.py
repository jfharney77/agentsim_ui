"""Degrading Analyst — LangGraph implementation.

Same Agent Card and skill as the normal LangGraph analyst. The hub/swarm sees
no difference in the A2A transport — discovery, skill validation, and message/send
all work identically.

The degradation happens INSIDE the StateGraph node: instead of a clean
llm.invoke(), the node routes through degrading_brain.degrading_think() which
wraps the LLM call with MAST failure injection that worsens over successive calls.

This mirrors the structure of analyst.py (LangGraph version) exactly:
  - Same StateGraph(MessagesState) pattern
  - Same START -> agent -> END edge topology
  - Same GraphExecutor wrapping it in an A2A server
  - But the 'agent' node degrades over time
"""
...
# Module-level degradation state — persists across calls to this server
_degradation_state = DegradationState()


def build_graph():
    """Build a LangGraph StateGraph with a degrading agent node.

    The graph topology is identical to the healthy analyst:
        START -> agent -> END

    But the 'agent' node uses degrading_think() instead of llm.invoke(),
    which means each invocation advances the degradation state and
    probabilistically injects MAST failure modes.
    """

    def degrading_node(state: MessagesState):
        """Graph node that degrades over successive calls.

        Extracts the user's text from the message state, runs it through
        degrading_think() (which wraps brain.think() with MAST failures),
        and returns the result as an AIMessage — exactly as the healthy
        analyst node would, but with progressive corruption.
        """
        ...
        # Route through degrading brain instead of direct LLM call
...
class DegradingGraphExecutor(AgentExecutor):
    """LangGraph executor with MAST degradation.

    Identical interface to the healthy GraphExecutor — the A2A server
...
if __name__ == "__main__":
    ...
    print(f"[Degrading Analyst/LangGraph] listening on {url}")
    print(f"[Degrading Analyst/LangGraph] card at {url}.well-known/agent-card.json")
    print(f"[Degrading Analyst/LangGraph] MAST failure injection ACTIVE")
