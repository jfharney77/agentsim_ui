...
def _clean_think_with_fallback(system_prompt: str, user_text: str, max_tokens: int = 800) -> str:
    """Wrapper around _clean_think with additional error handling."""
    try:
        return _clean_think(system_prompt, user_text, max_tokens)
    except Exception as e:
        # If there's still a null character error, return a simple fallback
        if "null" in str(e).lower():
            return "[LLM response contained invalid characters]"
        raise
...
def degrading_think(
    ...
    # Try the LLM call with error handling for null character issues
    try:
        result = _clean_think_with_fallback(system_prompt, user_text, max_tokens)
    except Exception as e:
        # If there's a null character error, provide a fallback response
        if "null" in str(e).lower():
            print(f"  [DEGRADATION] ⚠ LLM error (null character), using fallback response")
            # Return a safe fallback that allows degradation to continue
            return "[LLM ERROR: Response contained invalid characters. Using fallback response.]"
