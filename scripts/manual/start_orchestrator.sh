...
RESEARCHER_MOD=$(resolve_module simulators.manual.orchestrator researcher "${RESEARCHER_IMPL:-langgraph}")
ANALYST_MOD=$(resolve_module simulators.manual.orchestrator analyst "${ANALYST_IMPL:-langgraph}")
WRITER_MOD=$(resolve_module simulators.manual.orchestrator writer "${WRITER_IMPL:-langgraph}")
base_port="${PORT_BASE:-9101}"
echo "starting orchestrator workers:"
echo " researcher -> ${RESEARCHER_MOD}"
echo " analyst -> ${ANALYST_MOD}"
echo " writer -> ${WRITER_MOD}"
pids=()
cleanup() { echo; echo "stopping orchestrator workers..."; kill "${pids[@]}" 2>/dev/null || true; }
trap cleanup EXIT INT TERM
python -m "$RESEARCHER_MOD" & pids+=($!)
python -m "$ANALYST_MOD" & pids+=($!)
python -m "$WRITER_MOD" & pids+=($!)
echo "waiting for orchestrator workers to come online..."
for i in $(seq 1 20); do
  ...
  if [ "$up" -eq 3 ]; then
    echo "all 3 orchestrator workers up (ports ${base_port}-$((base_port + 2)))"
    echo "run the pipeline: ./scripts/manual/run_orchestrator.sh \"Acme Corp\""
    break
  fi
  sleep 1
done
